...
sys_exofork(void)
{
	...
	// set eax to zero so that child env would see env_id == 0
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if (((uintptr_t)va & (PGSIZE-1)) != 0 || (uintptr_t)va >= UTOP) {
		return -E_INVAL;
	}
	if ((perm | PTE_SYSCALL) != PTE_SYSCALL) {
		return -E_INVAL;
	}
	p = page_alloc(1); // initialized to 0
	if(p == NULL) {
		return -E_NO_MEM;
	}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	p = page_lookup(src_env->env_pgdir, srcva, &src_pte);
